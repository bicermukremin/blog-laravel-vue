<template>
  <section class="card">
    ...
    <div class="card-body" v-if="getBlogs.length">
      <div v-if="getBlogs.length">
        <table class="table table-bordered table-striped mb-0" id="datatable-editable">
          ...
          <tbody class="mb-5">
            <tr data-item-id="1" v-for="blog in getBlogs" :key="blog.id">
              <td>
                <img
                  width="50px"
                  height="50px"
              ...
              <td v-html="blog.description.substring(0, 50)"></td>
              <td class="actions">
                <a
                  href="#"
    ...
    <div v-else>
      <h1 class="text-center">Henüz Kaydedilmiş Bir Blog Bulunmamaktadır.</h1>
    </div>
    <!-- <div v-if="!completed"><loading :isLoading="true"></loading></div> -->

    <pagination
      v-if="getBlogs.length"
      :meta="getBlogMeta"
      @changePage="listBlogs"
    ></pagination>
    <BlogModal
      :name="name"
      :blog="blog"
...
export default {
  ...
  methods: {
    listBlogs(page) {
      ...
      this.$store.dispatch("initBlog", { filter: this.filter }).then(() => {
        this.completed = true;
      });
    },
    openModal() {
