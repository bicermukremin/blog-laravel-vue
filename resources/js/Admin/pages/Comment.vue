<template>
  <section class="card">
    ...
    <div class="card-body" v-if="getComments.length">
      <div v-if="getComments.length">
        <table class="table table-bordered table-striped mb-0" id="datatable-editable">
          ...
          <tbody class="mb-5">
            <tr data-item-id="1" v-for="comment in getComments" :key="comment.id">
    ...
    <div v-else>
      <h1 class="text-center">Henüz Kaydedilmiş Bir Comment Bulunmamaktadır.</h1>
    </div>
...
export default {
  ...
  computed: {
    ...mapGetters(["getComments", "getCommentErrors", "getCommentMeta"]),
  },
  watch: {
  ...
  methods: {
    listComments(page) {
      ...
      this.$store.dispatch("initComment", { filter: this.filter }).then(() => {
        this.completed = true;
      });
    },
    openModal() {
      ...
      this.$store
        ...
        .then(() => {
          ...
          if (this.getCommentErrors) {
            ...
            this.close();
            this.$toast.success(
              "Comment ekleme işlemi başarılı bir şekilde tamamlanmıştır."
            );
          }
        });
    },
      ...
      this.$store
        ...
        .then(() => {
          ...
          if (this.getCommentErrors) {
            ...
            this.$toast.success(
              "Comment güncelleme işlemi başarılı bir şekilde tamamlanmıştır."
            );
          }
        });
    },
