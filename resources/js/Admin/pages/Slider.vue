<template>
  <section class="card">
    ...
    <header class="card-header pr-15 pt-5">
      ...
      <h2 class="card-title">Slaytlar</h2>
    </header>
    <div class="card-body" v-if="getSliders.length">
      <div v-if="getSliders.length">
        <table class="table table-bordered table-striped mb-0" id="datatable-editable">
          ...
          <tbody class="mb-5">
            <tr data-item-id="1" v-for="slider in getSliders" :key="slider.id">
              <td>
                <img
                  width="50px"
                  height="50px"
                  :src="`http://127.0.0.1:8000/storage/${slider.image}`"
                  :alt="slider.title"
                />
              </td>
              <td>{{ slider.title }}</td>
    ...
    <pagination
      v-if="getSliders.length"
      :meta="getSliderMeta"
      @changePage="listSliders"
    ></pagination>
    <SliderModal
      :name="name"
      :slider="slider"
...
export default {
  ...
  computed: {
    ...mapGetters(["getSliders", "getSliderErrors", "getSliderMeta"]),
  },
  watch: {
  ...
  methods: {
    listSliders(page) {
      ...
      this.$store.dispatch("initSlider", { filter: this.filter }).then(() => {
        this.completed = true;
      });
    },
    openModal() {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.$store.dispatch("deleteSlider", id).then(() => {
            if (this.getSliderErrors) {
              inputError(Object.values(this.getSliderErrors));
            } else {
    ...
    createSlider(payload) {
      ...
      formData.append("title", payload.slider.title);

      /* debugger; */
      this.$store.dispatch("saveSlider", formData).then(() => {
    ...
    updateSlider(payload) {
      ...
      formData1.append("title", payload.slider.title);
      formData1.append("id", payload.slider.id);
