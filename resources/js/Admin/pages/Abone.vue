<template>
  <section class="card">
    ...
    <header class="card-header pr-15 pt-5">
      ...
      <h2 class="card-title">Abone</h2>
    </header>
    <div class="card-body" v-if="getAbones.length">
      <div v-if="getAbones.length">
        <table class="table table-bordered table-striped mb-0" id="datatable-editable">
          ...
          <tbody class="mb-5">
            <tr data-item-id="1" v-for="abone in getAbones" :key="abone.id">
              <td>{{ abone.email }}</td>

              <td class="actions">
                <a
                  href="#"
                  @click.prevent="editAbone(abone)"
                  class="on-default edit-row edit-buton"
                  ><i class="fas fa-pencil-alt"></i
                ></a>
                <a
                  href="#"
                  @click.prevent="deleteAbone(abone.id)"
    ...
    <div v-else>
      <h1 class="text-center">Henüz Kaydedilmiş Bir Abone Bulunmamaktadır.</h1>
    </div>
    <!-- <div v-if="!completed"><loading :isLoading="true"></loading></div> -->

    <pagination
      v-if="getAbones.length"
      :meta="getAboneMeta"
      @changePage="listAbones"
    ></pagination>
    <AboneModal
      :name="name"
      :abone="abone"
      :showImage="showImage"
      @close="close"
      @createAbone="createAbone"
      @updateAbone="updateAbone"
    ></AboneModal>
  </section>
</template>

<script>
import AboneModal from "./../components/AboneModal";
...
export default {
  ...
  computed: {
    ...mapGetters(["getAbones", "getAboneErrors", "getAboneMeta"]),
  },
  watch: {
  ...
  methods: {
    listAbones(page) {
      ...
      this.$store.dispatch("indexAbone", { filter: this.filter }).then(() => {
        this.completed = true;
      });
    },
    openModal() {
    ...
    deleteAbone(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.$store.dispatch("deleteAbone", id).then(() => {
            if (this.getAboneErrors) {
              inputError(Object.values(this.getAboneErrors));
            } else {
    ...
    createAbone(payload) {
      ...
      formData.append("email", payload.abone.email);

      /* debugger; */
      this.$store.dispatch("saveAbone", formData).then(() => {
        this.isLoading = false;
        if (this.getAboneErrors) {
          inputError(Object.values(this.getAboneErrors));
        } else {
          this.close();
          this.$toast.success("Abone ekleme işlemi başarılı bir şekilde tamamlanmıştır.");
        }
      });
    },
      ...
      this.$store
        ...
        .then(() => {
          ...
          if (this.getAboneErrors) {
            ...
            this.$toast.success(
              "Abone güncelleme işlemi başarılı bir şekilde tamamlanmıştır."
            );
          }
        });
    },
