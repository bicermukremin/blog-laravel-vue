<template>
  <section class="card">
    ...
    <header class="card-header pr-15 pt-5">
      ...
      <h2 class="card-title">Hizmetlerimiz</h2>
    </header>
    <div class="card-body" v-if="getServices.length">
      <div>
          ...
          <tbody class="mb-5">
            <tr data-item-id="1" v-for="(service, i) in getServices" :key="i">
              ...
              <td class="actions">
                ...
                <a
                  href="#"
                  @click.prevent="deleteService(service.slug)"
...
export default {
  ...
  computed: {
    ...mapGetters(["getServices", "getServiceErrors", "getServiceMeta"]),
  },
  watch: {
  ...
  methods: {
    listServices(page) {
      ...
      this.$store.dispatch("initService", { filter: this.filter }).then(() => {
        this.completed = true;
      });
    },
    openModal() {
    ...
    deleteService(slug) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.$store.dispatch("deleteService", slug).then(() => {
    ...
    createService(payload) {
      this.isLoading = true;
      /*  debugger; */
      var serviceItems = JSON.stringify(payload.items);
      const formData = new FormData();
      for (let i = 0; i < payload.images.length; i++) {
        formData.append("images[]", payload.images[i]);
      }
      formData.append("items", serviceItems);
      formData.append("title", payload.service.title);
      formData.append("content", payload.service.content);

      /* debugger; */
      this.$store.dispatch("saveService", formData).then(() => {
    ...
    updateService(payload) {
      ...
      if (payload.images.length > 0) {
        for (let i = 0; i < payload.images.length; i++) {
          formData1.append("images[]", payload.images[i]);
        }
      }

      formData1.append("title", payload.service.title);
      formData1.append("items", serviceItems);
      formData1.append("content", payload.service.content);
      formData1.append("id", payload.service.id);
      ...
      this.$store
        ...
        .then(() => {
          ...
          if (this.getServiceErrors) {
            ...
            this.$toast.success(
              "Hizmet güncelleme işlemi başarılı bir şekilde tamamlanmıştır."
            );
          }
        });
    },
